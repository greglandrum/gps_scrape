...
def estimate_profile(pts, nPtsToAvg=52):
    if not pts[0].findall('{http://www.topografix.com/GPX/1/1}ele'):
        return 0, 0, 0, 0
    elevs = [float(pt.findall(
        '{http://www.topografix.com/GPX/1/1}ele')[0].text) for pt in pts]
    gained = 0
    lost = 0
    maxEle = -10000
    minEle = 10000
    halfWindow = nPtsToAvg//2
    last = None
    for idx in range(halfWindow, len(pts), halfWindow):
        startIdx = idx-halfWindow
        endIdx = min(len(pts), idx+halfWindow)
        ...
        if last is not None:
            if mean >= last:
                ...
                lost += last - mean
        last = mean
    return gained, lost, maxEle, minEle
...
def doFile(name, options, idx, includeMarker=False):
    ...
    if not pts:
        ...
        g, l, maxV, minV = estimate_profile(pts)
        print(
            f" Max altitude {maxV}, min altitude {minV}, elev gained: {g}, elev lost: {l}")
    data = []
    for pt in pts:
...
procData = [doFile(arg, options, idx+1, idx)
            for (idx, arg) in enumerate(options.inFiles)]
